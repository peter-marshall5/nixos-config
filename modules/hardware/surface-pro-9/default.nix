...

{
  ...
  options.hardware.surface-pro-9.enable = lib.mkOption {
    type = lib.types.bool;
    default = false;
  };

  config = lib.mkIf config.hardware.surface-pro-9.enable {
    ...
    # DSP for internal speakers
    environment.etc = {
      "wireplumber/policy.lua.d/85-surface-policy.lua".source = ./sound/85-surface-policy.lua;
